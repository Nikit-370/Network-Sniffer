/*
 * This Text area shows the captured data in the hexadecimal format.Uses a method called showpacket() that actually 
 * shows the data in the packet 
 * 
 */
	...
	void showPacket(Packet p){
		...
		for(int i=0,j;i<bytes.length;){
			for(j=0;j<8 && i<bytes.length;j++,i++){
				String d=Integer.toString((int)(bytes[i]&0xff));
